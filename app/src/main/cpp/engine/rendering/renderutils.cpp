...
string RenderUtils::_glGetProgramInfoLog(GLuint program)
{
	GLchar buffer[1024];
	GLsizei length;
...
string RenderUtils::_glGetShaderInfoLog(GLuint shaderReference)
{
	GLchar buffer[1024];
	GLsizei length;

	glGetShaderInfoLog(shaderReference, 1024, &length, buffer);

	return string(buffer);
}

void RenderUtils::printUniforms(GLuint program)
{
	GLint uniformCount;
	...
	for (GLuint i = 0; i < uniformCount; i++) {
		...
		for (size_t j = 0; j < sizeof(type_set) / sizeof(glsl_type_set); j++) {
			...
			if (size > 1) {
				...
				stringStream << "Uniform " << i << " (loc=" << location << "):\t" << type_name << " " << name << " <Size: " << size << ">";
				Log::d("RenderUtils", stringStream.str());
			} else {
				ostringstream stringStream;
				stringStream << "Uniform " << i << " (loc=" << location << "):\t" << type_name << " " << name;
				Log::d("RenderUtils", stringStream.str());
			}
